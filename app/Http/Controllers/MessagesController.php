...
class MessagesController extends Controller
{
    ...
    public function idFetchData(Request $request)
    {
        ...
        if ($request['type'] == 'user') {
            $fetch = Caretaker::where('id', $request['id'])->first();
        }
        ...
        return Response::json([
            'favorite' => $favorite,
            'fetch' => $fetch,
            'user_avatar' => asset('/img/' . config('chatify.user_avatar.folder') . '/' . $fetch->avatar),
        ]);
    }
    ...
    public function getContacts(Request $request)
    {
        ...
        $users = Message::join('caretakers',  function ($join) {
            $join->on('messages.from_id', '=', 'caretakers.id')
                ->orOn('messages.to_id', '=', 'caretakers.id');
        })
        ...
        if ($users->count() > 0) {
            ...
            foreach ($users as $user) {
                if ($user->id != Auth::user()->id) {
                    // Get user data
                    $userCollection = Caretaker::where('id', $user->id)->first();
    ...
    {
        // Get user data
        $userCollection = Caretaker::where('id', $request['user_id'])->first();
    ...
    public function getFavorites(Request $request)
    {
        ...
        foreach ($favorites->get() as $favorite) {
            // get user data
            $user = Caretaker::where('id', $favorite->favorite_id)->first();
    ...
    public function search(Request $request)
    {
        ...
        $records = Caretaker::where('first_name', 'LIKE', "%{$input}%");
    ...
    public function updateSettings(Request $request)
    {
        ...
        if ($request['dark_mode']) {
            $request['dark_mode'] == "dark"
                ? Caretaker::where('id', Auth::user()->id)->update(['dark_mode' => 1])  // Make Dark
                : Caretaker::where('id', Auth::user()->id)->update(['dark_mode' => 0]); // Make Light
        }
        ...
        if ($request['messengerColor']) {
            ...
            Caretaker::where('id', Auth::user()->id)
        ...
        if ($request->hasFile('avatar')) {
            ...
            if ($file->getSize() < 150000000) {
                if (in_array($file->getClientOriginalExtension(), $allowed_images)) {
                    ...
                    $update = Caretaker::where('id', Auth::user()->id)->update(['avatar' => $avatar]);
    ...
    public function setActiveStatus(Request $request)
    {
        $update = $request['status'] > 0
            ? Caretaker::where('id', $request['user_id'])->update(['active_status' => 1])
            : Caretaker::where('id', $request['user_id'])->update(['active_status' => 0]);
        // send the response
        return Response::json([
            'status' => $update,
        ], 200);
